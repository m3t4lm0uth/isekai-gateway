...
public class GatewayDatabase {
    ...
    public List<EventSummary> listEvents() throws GatewayDatabaseException {
        ...
        try (Connection conn = dataSource.getConnection();
            ...
            while (rs.next()) {
                ...
                String clears = rs.getString("clears");

                result.add(new EventSummary(key, name, status, backend, priority, triggers, clears));
            }
        } catch (SQLException ex ) {
            throw new GatewayDatabaseException(
    ...
    public List<PlayerFlagSummary> listPlayerFlags(PlayerFlagFilter filter) throws GatewayDatabaseException {
        String baseSql = """
                select
                    f.player_uuid,
                    e.event_key,
        ...
        switch (filter) {
            case ACTIVE -> sql = baseSql + " where f.required = true order by f.player_uuid, e.event_key";
            case ALL -> sql = baseSql + " order by f.player_uuid, e.event_key";
            default -> throw new IllegalArgumentException("Unhandled filter: " + filter);
        }

        List<PlayerFlagSummary> result = new ArrayList<>();
        ...
        try (Connection conn = dataSource.getConnection();
            ...
            while (rs.next()) {
                ...
                String eventKey = rs.getString("event_key");
                boolean required = rs.getBoolean("required");
                Timestamp requiredAt = rs.getTimestamp("required_at");
                Timestamp clearedAt = rs.getTimestamp("cleared_at");
                result.add(new PlayerFlagSummary(playerUuid, eventKey, required, requiredAt, clearedAt));
            }
        } catch (SQLException ex ) {
            throw new GatewayDatabaseException(
                    "Failed to list player flags.", ex
            );
        }
    ...
    public void triggerMarkEventsRequired(UUID playerUuid, String actionKey) throws GatewayDatabaseException, EventNotFoundException {
        ...
        try (Connection conn = dataSource.getConnection();
            ...
            try (ResultSet rs = ps.executeQuery()) {
                while (rs.next()) {
                    eventKeys.add(rs.getString("event_key"));
                }
            }
        ...
        for (String eventKey : eventKeys) {
            markEventRequired(playerUuid, eventKey);
        }
    ...
    // ------- player flags -------
    // flag event as required for player
